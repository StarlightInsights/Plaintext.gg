...
test.describe("Toolbar visibility", () => {
  ...
  test("editor padding adjusts when toolbar visibility changes", async ({ page }) => {
    await page.goto("/");
    const editor = page.locator("#editor");
    const hiddenPadding = await editor.evaluate((el) =>
      parseInt((el as HTMLElement).style.paddingTop),
    );
    expect(hiddenPadding).toBe(40);
    await page.locator("#btn-toggle-desktop").click();
    await page.waitForTimeout(50);
    const visiblePadding = await editor.evaluate((el) =>
      parseInt((el as HTMLElement).style.paddingTop),
    );
...
test.describe("Dialog layout", () => {
  ...
  test("weight buttons do not fill the full width", async ({ page }) => {
    ...
    const groupWidth = await weightSetting
      .locator(".setting-control--group")
      .evaluate((el) => (el as HTMLElement).offsetWidth);
...
test.describe("Accessibility", () => {
  ...
  test("controls group has correct aria-label", async ({ page }) => {
    await page.goto("/");
    await expect(page.locator(".toolbar-controls")).toHaveAttribute(
      "aria-label",
      "Editor controls",
    );
  });
...
test.describe("File upload", () => {
  ...
  test("drag leave removes visual feedback", async ({ page }) => {
    ...
    await editor.evaluate((el) => {
      el.dispatchEvent(
        new DragEvent("dragleave", { bubbles: true, relatedTarget: null } as DragEventInit),
      );
    });
